...
class GoldenThroneHandler extends BaseItemHandler
{
    ...
    public function validate(UserItem $userItem, User $user, ?User $target, League $league): void
    {
        ...
        if (! $effectId) {
            throw ValidationException::withMessages([
                'effect_id' => ['You must select an effect to negate.'],
            ]);
        }
        ...
        if (! $effect) {
            throw ValidationException::withMessages([
                'effect_id' => ['The selected effect does not exist or cannot be negated.'],
            ]);
        }
    }
    ...
    public function getUserNotification(ItemEffect $effect, ?User $target): ?array
    {
        return [
            'title' => 'Golden Throne',
            'body' => 'Effect negated and +3% step boost gained!',
        ];
    }
}
